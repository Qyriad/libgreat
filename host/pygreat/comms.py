...
class CommsBackend(object):
    ...
    def generate_async_api_object(self):
        """ Generates an object that gives us a view of all asynchronous API methods
            accessible via communciations API. Identical to touching the .apis
            dictionary, but uses object semantics.
        """

        # Generate a class that wraps self.async_apis, so self.async_apis can be easily
        # accessed with object semantics.
        return CommsApiCollection(self.async_apis)
    ...
    def _generate_object_for_class(self, class_number, overwrite=False):
        ...
        # class's methods.
        attrs, async_attrs = self._generate_rpc_verbs_for_class(class_number, class_name)

        # Each comms class needs a CLASS_NUMBER attribute, and should have a
        # CLASS_NAME attribute. We'll add ours.
        attrs['CLASS_NUMBER'] = async_attrs['CLASS_NUMBER'] = class_number
        attrs['CLASS_NAME'] = async_attrs['CLASS_NAME'] = class_name
        attrs['IS_ASYNC'] = False
        async_attrs['IS_ASYNC'] = True

        # Generat a documentation string for the given class.
        attrs['__doc__'] = async_attrs['__doc__'] = \
        ...
        async_cls = type(class_name, (GeneratedCommsClass,), async_attrs)

        # Finally, instantiate and store the relevant class.
        self.apis[class_name] = cls(self)
        self.async_apis[class_name] = async_cls(self)
    ...
    def _generate_rpc_verbs_for_class(self, class_number, class_name = "class"):
        ...
        for verb_number in verb_numbers:
            ...
            rpcs[name], async_rpcs[name] = command_rpc(verb_number, in_signature, out_signature, name=name, class_name=class_name,
                    doc=documentation, in_parameter_names=in_param_names, out_parameter_names=out_param_names)

        return rpcs, async_rpcs
    ...
    async def async_execute_command(self, class_number, verb, in_format, out_format,
            *arguments, **kwargs):
        """Executes a libgreat command asynchronously.
        ...
        raw_result = await self.async_execute_raw_command(class_number, verb, payload, timeout,
                None, max_response_length, comms_timeout, pretty_name, rephrase_errors)
        ...
        # If we have an encoding, convert any byte arguments into a string.
        if encoding:
    ...
    async def execute_raw_command(self, class_number, verb, data=None, timeout=1000, encoding=None,
            max_response_length=4096, comms_timeout=1000, pretty_name='unknown', rephrase_errors=True):
        """Executes a libgreat command.
...
def command_rpc(verb_number, in_format="", out_format="", name="function", class_name="class",
    ...
    # Create an asynchronous version of the same thing.
    async def async_method(self, *arguments, **kwargs):
        encoding = kwargs.pop('encoding', None)
        timeout  = kwargs.pop('timeout', 1000)
        max_response_length = kwargs.pop('max_response_length', 4096)

        return await self.async_execute_command(verb_number, in_format, out_format, name=name, class_name=class_name,
                timeout=timeout, max_response_length=max_response_length, encoding=encoding, *arguments)
    ...
    async_method.__name__ = future_utils.native_str(name)
    ...
    try:
        ...
        async_method.__signature__ = _generate_command_rpc_signature(
                in_format, in_parameter_names, out_format, out_parameter_names)
    except AttributeError:
        pass

    return method, async_method
...
class CommsClass(object):
    ...
    def async_execute_command(self, verb, in_format, out_format, *arguments, **kwargs):
        """Executes a libgreat command.
        ...
        Returns any data recieved in response, parsed according to out_format.
        """
        return self.comms_backend.async_execute_command(self.CLASS_NUMBER, verb, in_format,
                out_format, *arguments, **kwargs)
    ...
    def async_execute_raw_command(self, class_number, verb, data=None, timeout=1000, encoding=None,
            max_response_length=4096, comms_timeout=1000, pretty_name="unknown", rephrase_errors=True):
        """Executes a libgreat command.
        ...
        Returns any data recieved in response.
        """
        return self.async_execute_raw_command(self.CLASS_NUMBER, verb, data, timeout,
                encoding, max_response_length, comms_timeout, pretty_name, rephrase_errors)
